  ...
  return (
    <aside className="about-panel" role="dialog" aria-modal="true" aria-labelledby="about-title">
      <div className="drawer-header">
        <div><p className="eyebrow">Methods & provenance</p><h2 id="about-title">About the data</h2></div>
        <button className="icon-button" onClick={onClose} aria-label="Close about panel"><X /></button>
      </div>
      <div className="about-content">
        <p className="about-lede">An exhaustive, paragraph-level catalogue of named human settlements in the substantive text and notes of <em>The Dawn of Everything</em>.</p>
        ...
        <section><h3>Reading the dates</h3><p>Occupation dates summarize known periods of habitation rather than claiming unbroken occupation. They are a reference synthesis for this dataset and are not necessarily dates stated in the cited book passage.</p></section>
        ...
        <div className="validation-callout"><CheckCircle2 /><span><strong>Validated source fidelity</strong>The current release passes all 20 relational, range, citation-link and source-fidelity checks.</span></div>
        <section><h3>Build record</h3><ul>{dataset.qa.map((item) => <li key={item.check}><span>{item.check}</span><b>{item.value}</b></li>)}</ul></section>
        ...
        <p className="attribution">Settlement text and metadata are presented for research and exploration. The atlas is an independent project and is not affiliated with the book’s authors or publisher.</p>
      </div>
    </aside>
  )
}
