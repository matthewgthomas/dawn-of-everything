...
describe('App responsive results', () => {
  ...
  it('retargets the desktop results launcher to the Book mentions panel', async () => {
    ...
    const user = userEvent.setup()
    render(<App />)
    const launcher = screen.getByRole('button', { name: '144 settlements' })
  ...
  it('switches between timeline and settlement-area modes without replacing the timeline component', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Settlement area' }))
    expect(screen.getByRole('heading', { name: 'Settlement area' })).toBeInTheDocument()
    expect(screen.getByText('69 of 144 filtered settlements have an estimated size.')).toBeInTheDocument()
  ...
  it('builds a named settlement allow-list from mouse and keyboard suggestions across every view', async () => {
    ...
    expect(screen.getByRole('button', { name: '144 settlements' })).toBeInTheDocument()
...
describe('SettlementAreaComparison', () => {
  it('ranks known areas and retains unknown settlements in a separate group', async () => {
    ...
    expect(rows[0]).toHaveTextContent('Burning Man’s Black Rock City')
    expect(rows[0]).toHaveTextContent('Heathrow Airport')
    expect(rows[1]).toHaveTextContent('About the same area as the Glastonbury Festival site')
...
describe('DetailDrawer', () => {
  ...
  it('summarizes peak area and exposes full comparator provenance', async () => {
    ...
    expect(screen.getByText('About 1.1 × the area of Burning Man’s Black Rock City')).toBeInTheDocument()
    expect(screen.getByText('About 1.7 × the area of Heathrow Airport')).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Area' }))
    expect(screen.getByRole('heading', { name: 'Settlement area' })).toBeInTheDocument()
    expect(screen.getByText('20.7 km²')).toBeInTheDocument()
    const comparator = screen.getByRole('link', { name: /Burning Man’s Black Rock City/i })
    expect(comparator).toHaveAttribute('href', 'https://survival.burningman.org/law-enforcement/specific-laws-to-be-aware-of/')
    expect(comparator).toHaveAttribute('target', '_blank')
    expect(screen.getByRole('link', { name: /Heathrow Airport/i })).toHaveAttribute('href', 'https://www.heathrow.com/company/about-heathrow/company-faqs')
    expect(screen.getByText(/Whole event area including open playa, not just the street grid/)).toBeInTheDocument()
